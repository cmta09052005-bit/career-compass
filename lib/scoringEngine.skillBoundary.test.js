...
assert.equal(c1Courses["BSIT-001"].finalCourseMatchPercent, 88);
assert.equal(c1Courses["BSIS-001"].componentA, 4);
assert.equal(c1Courses["BSIS-001"].finalCourseMatchPercent, 86);
assert.equal(c1Courses["BSCpE-001"].finalCourseMatchPercent, 82);
assert.equal(c1Courses["BSCS-001"].finalCourseMatchPercent, 82);

console.log("PASS: SKL-01=4 adds 2 to CategoryScore(C1) and gives BSIS-001 componentA=4.");
console.log("PASS: exact reproduction produces C1=88%, BSIT=88%, BSIS=86%, BSCpE=82%, BSCS=82%.");
