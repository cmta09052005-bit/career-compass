...
const DRAW_TEXT = Symbol("localizedReportText");
...
function addWrappedText(doc, text, x, y, maxWidth, options = {}) {
  ...
  for (const line of lines) {
    ...
    doc[DRAW_TEXT](line, x, y, options);
    y += 5;
  }
  return y;
}
...
export function createExplorerReport(report) {
  ...
  const localize = value => Array.isArray(value) ? value.map(localize) : printableText(translate(value, language));
  // Translate before measuring/wrapping, so longer Filipino copy paginates correctly.
  const splitText = doc.splitTextToSize.bind(doc);
  const drawText = doc.text.bind(doc);
  doc.splitTextToSize = (text, ...args) => splitText(localize(text), ...args);
  doc[DRAW_TEXT] = drawText;
  doc.text = (text, ...args) => drawText(Array.isArray(text) ? text : localize(text), ...args);
  ...
  rankedCourses.forEach((course, index) => {
    ...
    const nameLines = doc.splitTextToSize(`Rank #${index + 1}: ${course.courseName}`, 162);
  ...
  doc.setFontSize(8.5);
  y = addWrappedText(doc, academicLine, 20, y, 170);
  doc.setFont("helvetica", "italic");
  doc.setFontSize(8);
  y = addWrappedText(doc, "Keep this report as a separate copy. Answers are saved only in this browser on this device.", 20, y + 3, 170);
...
export function buildExplorerReport(session, answers, result, language = "en") {
  return {
    language,
    nickname: session.nickname,
    date: session.reportDate || new Intl.DateTimeFormat("en-PH", { dateStyle: "long" }).format(new Date()),
    answers,
...
export async function downloadExplorerReport(report) {
  const doc = createExplorerReport(report);
  await doc.save("career-compass-explorer-report.pdf", { returnPromise: true });
}
