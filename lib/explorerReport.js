...
export function createExplorerReport(report) {
  ...
  y = addSectionHeading(doc, "Your Career Map", y);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(20);
  doc.setTextColor(...COLORS.navy);
  y = addWrappedText(doc, report.topCategory.label, 20, y, 145);
  doc.setFillColor(...COLORS.gold);
  doc.roundedRect(166, 27, 24, 13, 4, 4, "F");
  doc.setFontSize(13);
  doc.setTextColor(...COLORS.navy);
  doc.text(`${report.topCategory.percentage}%`, 178, 35.5, { align: "center" });

  y += 7;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(11);
  doc.setTextColor(...COLORS.muted);
  doc.text("Top recommended courses", 20, y);
  y += 7;
  report.rankedCourses.slice(0, 4).forEach((course, index) => {
    doc.setFont("helvetica", "bold");
    doc.setTextColor(...COLORS.navy);
    doc.text(`${index + 1}. ${course.courseName}`, 24, y);
    doc.setTextColor(...COLORS.gold);
    doc.text(`${course.matchPercentage}%`, 190, y, { align: "right" });
    y += 7;
  });
  ...
  y = addSectionHeading(doc, "Ranked Recommended Paths", y);
  report.rankedCourses.forEach((course, index) => {
    y = addPageIfNeeded(doc, y, 21);
    doc.setFillColor(244, 246, 249);
    doc.roundedRect(20, y - 5, 170, 15, 3, 3, "F");
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    doc.setTextColor(...COLORS.navy);
    doc.text(`${index + 1}. ${course.courseName}`, 25, y + 3);
    doc.setTextColor(...COLORS.gold);
    doc.text(`${course.matchPercentage}% match`, 185, y + 3, {
      align: "right",
    });
    y += 20;
  });
