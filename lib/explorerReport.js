...
doc.setFontSize(13);
doc.setTextColor(...COLORS.navy);
doc.text(`Prepared for ${displayName}`, 105, 178, {
align: "center",
});

doc.setFontSize(11);
doc.setTextColor(...COLORS.muted);
doc.text(report.date || "", 105, 187, {
align: "center",
});
...
doc.setFontSize(10);
doc.text(
"Your journey is mapped. Your next step is yours to explore.",
105,
216,
{
align: "center",
...
.sort(
(a, b) =>
b.finalCourseMatchPercent - a.finalCourseMatchPercent,
)
.slice(0, 5);
...
const rowHeight =
  nameLines.length * 5 + (index === 0 ? 16 : 10);
...
addWrappedText(
doc,
`Supporting answers for ${
        topCourse?.categoryName || "your top category"
      }`,
20,
y,
170,
...
for (const entry of entries.length
  ? entries
  : ["No qualifying answers selected."]) {
  y = addWrappedText(
    doc,
    `- ${entry}`,
    24,
...
const academicLine = printableText(
`Academic: Strand: ${
      profile.strand || "Not provided"
    } | GWA: ${
      profile.gwa ?? "Not provided"
    } | Subjects: ${
...
Math.min(
8.5,
(8.5 * 169) / doc.getTextWidth(academicLine),
),
);
...
y = addWrappedText(
doc,
"Your answers are not stored anywhere - this report is the only copy.",
20,
y + 3,
170,
);
...
(course) => course.courseId === topCourse?.courseId,
);
...
const sections = [
  ...
  [
    ...
    details.careerOpportunities.map(
      (career) =>
        `${career.jobTitle} - ${career.salaryRange}${
