 ...
 * Rules for all six categories are finalized in docs/scoring-table.md Section 6.
 *
...
function courseTieBreakerBonus(categoryId, courseId, answers) {
  ...
  switch (categoryId) {
    case "C1": {
      // Finalized C1 course signals for IT & Computing.
      switch (courseId) {
        case "BSIT-001":
        ...
        case "BSCS-001":
          return hasSubject(input, "Statistics/Research")
            ...
            : 0;
        case "BSIS-001": {
          const slider = Number(input.skills?.["SKL-01"]);
    ...
    // Finalized C2 course signals for Engineering & Technology.
    case "C2": {
      switch (courseId) {
        case "BSCE-001": {
    ...
    // Finalized C3 course signals for Business Administration & Related.
    case "C3": {
      switch (courseId) {
        case "BSBA-001": {
    ...
    // Finalized C4 course signals for Education Science & Teacher Training.
    case "C4": {
      switch (courseId) {
        case "BEED-001": {
        ...
        case "BSED-001":
          return hasSubject(input, "English/Communication Arts")
            ...
            : 0;
        case "BTLE-001": {
          const slider = Number(input.skills?.["SKL-03"]);
          return Number.isFinite(slider) && slider >= SKILLS_WEIGHTS.threshold
            ? TIE_BREAKER_BONUS
            : 0;
        }
        case "BPED-001":
          return hasSubject(input, "Filipino/Araling Panlipunan")
            ? TIE_BREAKER_BONUS
            : 0;
        default:
          return 0;
    ...
    // Finalized C5 course signals for Medical & Allied Health Sciences.
    case "C5": {
      switch (courseId) {
        case "BSN-001": {
        ...
        case "BSPharma-001":
          return hasSubject(input, "PE & Health")
            ? TIE_BREAKER_BONUS
            : 0;
        case "BSMLS-001":
          return hasSubject(input, "Biology/General Science")
            ...
            : 0;
        case "BSRT-001": {
          const slider = Number(input.skills?.["SKL-05"]);
          return Number.isFinite(slider) && slider >= SKILLS_WEIGHTS.threshold
            ? TIE_BREAKER_BONUS
            : 0;
        }
    ...
    // Finalized C6 course signals for Architecture, Fine Arts & Design.
    case "C6": {
      switch (courseId) {
        case "BSArch-001":
          return hasSubject(input, "Physics") ? TIE_BREAKER_BONUS : 0;
        case "BFA-001":
          return hasSubject(input, "MAPEH-Arts")
            ? TIE_BREAKER_BONUS
            : 0;
        case "BSID-001":
