 ...
 * Only C1 rules are finalized (docs/scoring-table.md Section 6).
 * C2–C6 stay at 0 until documented — do not invent rules here.
 *
...
function courseTieBreakerBonus(categoryId, courseId, answers) {
  ...
  switch (categoryId) {
    ...
    // FUTURE C2 (Engineering & Technology) — add cases here, same pattern as C1.
    // Courses: BSCE-001, BSEE-001, BSECE-001, BSME-001
    // Example once a rule is documented:
    //   case "BSCE-001":
    //     return hasSubject(input, "<documented subject>") ? TIE_BREAKER_BONUS : 0;
    case "C2":
      return 0;

    // FUTURE C3 (Business Administration & Related) — add cases here, same pattern as C1.
    // Courses: BSBA-001, BSA-001, BSENT-001, BSHRM-001
    case "C3":
      return 0;

    // FUTURE C4 (Education Science & Teacher Training) — add cases here, same pattern as C1.
    // Courses: BEED-001, BSED-001, BTLE-001, BPED-001
    case "C4":
      return 0;

    // FUTURE C5 (Medical & Allied Health Sciences) — add cases here, same pattern as C1.
    // Courses: BSN-001, BSPharma-001, BSMLS-001, BSRT-001
    case "C5":
      return 0;

    // FUTURE C6 (Architecture, Fine Arts & Design) — add cases here, same pattern as C1.
    // Courses: BSArch-001, BFA-001, BSID-001, BSMA-001
    case "C6":
      return 0;
 ...
 * Each course starts at the category Match %, then C1 may add a documented
 ...
 * C2–C6 use the flat category % until their rules are finalized.
 *
