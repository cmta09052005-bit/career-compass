...
function courseComponents(categoryId, courseId, answers) {
  ...
  return {
    ...
    componentB,
    componentC,
    courseBonusPoints: componentA + componentB + componentC,
  };
}
 ...
 * @property {number} [scaledCategoryBase] Debug-level category Match % × 0.90
 ...
 * @property {number} [courseBonusPoints] Debug-level sum of components A, B, and C
 * @property {number} rawScoreBeforeRounding Uncapped scaled base plus bonus
 * @property {number} finalCourseMatchPercent Rounded and capped at 100
 */
...
export function rankAllCourses(courseMatches) {
  return [...(courseMatches ?? [])].sort((a, b) => {
    if (b.finalCourseMatchPercent !== a.finalCourseMatchPercent) {
      return b.finalCourseMatchPercent - a.finalCourseMatchPercent;
    }
 ...
 * Each course uses 90% of its category's raw Match % as its base, then adds
 * the course's graduated primary, strand, and applicable GWA components.
 *
 * @param {CategoryMatchPercentage[]} [categoryPercentages]
 * @param {AssessmentAnswers} [answers]
...
export function calculateAllCourseMatches(
  ...
  const courseMatches = (courseList ?? []).map((course) => {
    ...
    return {
      ...
      finalCourseMatchPercent: Math.round(
        Math.min(100, rawScoreBeforeRounding),
      ),
    };
  });

  return rankAllCourses(courseMatches);
}
 ...
 * Each course starts at 90% of the category Match %, then adds its Option C
