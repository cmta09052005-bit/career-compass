...
const cases = [
  {
    ...
    run() {
      ...
      assert.equal(course(result, "BSIT-001").scaledCategoryBase, 75.60000000000001);
      assert.equal(course(result, "BSIT-001").courseBonusPoints, 10);
      assert.equal(course(result, "BSIT-001").finalCourseMatchPercent, 86);
      ...
      for (const courseId of expectedOrder.slice(1)) {
        ...
        assert.equal(match.rawScoreBeforeRounding, 79.60000000000001);
        assert.equal(match.finalCourseMatchPercent, 80);
      }
  ...
  },
  {
    ...
    run() {
      ...
      // Every C5 course's primary signal is maximized here, so identical raw
      // and displayed scores legitimately fall back to alphabetical order.
      assert.deepEqual(
      ...
      for (const courseId of expectedOrder) {
        ...
        assert.equal(match.courseBonusPoints, 9);
        assert.equal(match.rawScoreBeforeRounding, 96);
        assert.equal(match.finalCourseMatchPercent, 96);
      }
  ...
  },
  {
    ...
    run() {
      ...
      for (const match of c6Courses) {
        assert.equal(match.scaledCategoryBase, 57.6);
        assert.equal(match.componentB, 2);
        assert.equal(match.componentC, 0);
      }
      assert.equal(byId["BFA-001"].rawScoreBeforeRounding, 65.6);
      assert.equal(byId["BSID-001"].rawScoreBeforeRounding, 65.6);
  ...
  // A student who maximizes every scoring input for a category will naturally satisfy every course's tie-breaker rule under that category, since course tie-breakers are drawn from the same interests/skills/subjects that feed the category score. This is not a bug — ties are resolved via the existing raw-score-then-alphabetical rule.
  {
    ...
    run() {
      ...
      for (const { courseId } of expectedTieBreakers) {
        ...
        assert.equal(match.courseBonusPoints, 10);
        assert.equal(match.rawScoreBeforeRounding, 100);
        assert.equal(match.finalCourseMatchPercent, 100);
      }
