...
function runTieBreakerCheck(category, expectedCourseId, answers) {
  ...
  const expectedMatch = answers.subjects ? 68 : 66;
  const expectedPercents = answers.subjects
    ? [68, 62, 62, 62]
    : category === "C5"
      ? [66, 62, 62, 62]
      : [66, 63, 62, 62];
  ...
  const pass =
    ...
    courses.every(
      (course, index) => course.matchPercentage === expectedPercents[index],
    );
...
function logResult(result) {
  ...
  if (top.category === "C1") {
    ...
    const differentiatedOk =
      bsit === 100 && bscs === 98 && bsis === 98 && bscpe === 98;
    console.log("\nC1 secondary-evidence check:");
    console.log(
      `  BSIT receives its secondary signal while category-mates do not: ${differentiatedOk ? "PASS" : "FAIL"} (${bsit}/${bscs}/${bsis}/${bscpe})`,
    );
    console.log(
      `  NOTE: exact remaining ties use raw score and alphabetical order: [${ids.join(", ")}].`,
    );
  } else {
...
const optionCChecks = [
  ...
  (() => {
    ...
    return match.scaledCategoryBase === 83.6 &&
      match.courseBonusPoints === 10 &&
      match.rawScoreBeforeRounding === 93.6 &&
      match.finalCourseMatchPercent === 94;
  })(),
  singleCourseMatch("C1", "BSIT-001", 100, {
    skills: { "SKL-01": 5 },
    strand: "Academic-STEM",
    gwa: 92,
...
function logTieBreakerCheck(result) {
  const percentages = result.courses
    ...
    .join(", ");
  console.log(
    `\n>>> ${result.pass ? "PASS" : "FAIL"}: ${result.category} evidence differentiates ${result.expectedCourseId} at ${result.expectedMatch}% (${percentages})`,
  );
}
