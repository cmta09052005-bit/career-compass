...
function runTieBreakerCheck(category, expectedCourseId, answers) {
  ...
  const pass =
    ...
    byId[expectedCourseId]?.matchPercentage === 75 &&
    categoryMates.every((course) => course.matchPercentage === 70) &&
    uniqueCoursePercents(courses).length === 2;

  return { category, expectedCourseId, courses, pass };
}
...
const tieBreakerChecks = [
  ...
  runTieBreakerCheck("C3", "BSBA-001", { skills: { "SKL-04": 4 } }),
...
function logTieBreakerCheck(result) {
  const percentages = result.courses
    ...
    .join(", ");
  console.log(
    `\n>>> ${result.pass ? "PASS" : "FAIL"}: ${result.category} tie-breaker boosts ${result.expectedCourseId} to 75% while category-mates remain at 70% (${percentages})`,
  );
}
